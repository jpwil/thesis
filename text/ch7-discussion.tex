...
In these contexts, early mortality \textemdash\ the most common cause of initial treatment failure \textemdash\ occurs due to advanced disease at presentation with severe organ dysfunction. Consequently, and as identified in Chapter~\ref{ch:3-sys-review}, risk factors for mortality reflect the degree of organ dysfunction at presentation. These patients never reach a post-treatment state in which relapse can occur. Extrapolating predictors of initial treatment failure from these settings to relapse is therefore mechanistically inappropriate, given the distinct underlying pathophysiology.
...
Verrest et al.\ (2024)\cite{verrest2024} hypothesised that post-treatment parasite dynamics are influenced by four key factors: (i) the pre-treatment parasite burden, (ii) the degree of parasite depletion by the treatment, (iii) the intrinsic parasite virulence, and, crucially, (iv) the host's immune system. It can be further hypothesised that, during the post-treatment stage, for each individual there must exist a `critical parasite load' (Figure~\ref{fig:natural-history}). The threshold itself would be expected to vary between individuals, reflecting both differences in host immunity and in parasite virulence. For parasite loads below this threshold, the host immune system controls parasite replication resulting in lasting cure. For parasite loads above this threshold, parasite replication outpaces immune control, leading to relapse (Figure~\ref{fig:natural-history}).

\subsubsection{Development of a Causal Model for Relapse}

Building on Verrest et al.'s hypothesis, a causative (mechanistic) model can be assembled. In this putative model, three key drivers of relapse, all shaped by events prior to and during treatment, determine whether relapse occurs: (i) post-treatment parasite grade, (ii) host immunity, and (iii) parasite fitness. Post-treatment parasite grade itself can be considered a direct consequence of both pre-treatment parasite grade and the treatment regimen. If presented as a directed acyclic graph (DAG), these drivers represent the final common pathway (mediators) of relapse. As shown in Figure~\ref{fig:dag}, text boxes (`nodes') represent relapse determinants, and lines (`vertices') indicate the direction of causation between nodes. For example, pre-treatment host immunity directly affects the initial rate of symptom progression.

Caution is, however, warranted when interpreting such a model. As with DAGs in any disease process, it represents a broad approximation of true biological events and fails to capture the dynamic interactions between the identified determinants as the disease develops. Rarely do disease experts agree on the `correct' DAG\cite{daveysmith2016}. Limitations aside, the proposed DAG at least provides an explicit conceptual framework that contextualizes the predictors identified in this thesis, and clarifies how they may relate mechanistically to relapse.
...
In both regions, higher pre-treatment parasite grade was strongly associated with increased relapse risk. For each unit increase in parasite grade (e.g. from 1+ to 2+), the odds of relapse increased by 31\% in the ISC (OR 1.31, 95\% CI 1.12--1.52) and 43\% in East Africa (OR 1.43, 95\% CI 1.23--1.66).
