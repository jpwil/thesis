...
An often cited `rule of thumb' is that at least 10--20 outcome events per predictor parameter (EPP) are required to prevent model overfitting\cite{peduzzi1996,harrell2015}. However, both the validity of this threshold and the broader premise that a single rule applies universally have been increasingly questioned in the prediction modelling literature\cite{courvoisier2011}.
...
Candidate predictors available at the time of starting treatment were selected based on (i) existing evidence and expert opinion regarding their association with relapse\cite{hirve2016, goyal2020, burza2014}, as summarised in Chapter \ref{ch:2-background}, Section \ref{sec:relapse}, (ii) availability across the contributed datasets; and (iii) feasibility for routine use outside research settings.
